...
COLUMN = 1000
ROW = 1000
GRID = Array.new(COLUMN){Array.new(ROW, 0)}

def parse_instructions(instruction_to_parse)
    instruction = instruction_to_parse.match(/(turn off|turn on|toggle) (\d+),(\d+) through (\d+),(\d+)/)
    return {type: instruction[1], x1: instruction[2].to_i, y1: instruction[3].to_i, x2: instruction[4].to_i, y2: instruction[5].to_i}
end

def follow_instructions(type, x1, x2, y1, y2)
    case type
    when "turn off"
        for row in x1..x2
            for col in y1..y2
...
INSTRUCTIONS.split("\n").each do |instruction|
    parsed_instruction = parse_instructions(instruction)
    type = parsed_instruction[:type]
    x1 = parsed_instruction[:x1]
    x2 = parsed_instruction[:x2]
    y1 = parsed_instruction[:y1]
    y2 = parsed_instruction[:y2]
    follow_instructions(type, x1, x2, y1, y2)
end
    
p GRID.reduce(0) {|acc, row| acc += row.sum()}